...
def calculate(modelPath, planes, origin_offset, origin_x_offsets=0):
    ...
    App.getDocument("model").Part__Feature.Placement = App.Placement(App.Vector(-900, 800, -500),
    ...
    App.getDocument('model').getObject('DatumPoint').Support = [
        (App.getDocument('model').getObject('Part__Feature'), 'Face75'),
        (App.getDocument('model').getObject('Y_Axis002'), '')]
